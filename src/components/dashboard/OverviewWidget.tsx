...
export const OverviewWidget: React.FC<OverviewWidgetProps> = ({ 
  ...
  return (
    <div className="bg-zinc-900 p-6 rounded-3xl shadow-sm border border-zinc-800 flex flex-col gap-6">
      <div className="flex flex-col md:flex-row items-center gap-6">
        <div className="relative w-24 h-24 flex-shrink-0">
          <svg className="w-full h-full transform -rotate-90">
            ...
            <circle cx="48" cy="48" r="40" stroke="currentColor" strokeWidth="8" fill="transparent" strokeDasharray={251.2} strokeDashoffset={251.2 * (1 - dailyEfficiency / 100)} className="text-white transition-all duration-1000" strokeLinecap="round" />
          </svg>
          <div className="absolute inset-0 flex items-center justify-center flex-col">
            <span className="text-xl font-bold text-white">{dailyEfficiency}%</span>
          </div>
        </div>
        <div className="flex-1 text-center md:text-left">
          <h2 className="text-lg font-bold text-white mb-1">Обзор дня и Эффективность</h2>
          ...
          <div className="flex justify-center md:justify-start gap-4 mt-3">
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-[10px] font-bold text-white">{completedTasks} задач</span>
            </div>
            <div className="flex items-center gap-1.5">
              <Award className="w-3 h-3 text-emerald-500" />
              <span className="text-[10px] font-bold text-white">{doneHabits.length} привычек</span>
            </div>
          </div>
        </div>
      </div>
        ...
        {totalGoodHabits > 0 && (
          <div className="space-y-1.5">
            <div className="flex justify-between text-[10px] uppercase tracking-wider">
              ...
              <span className="text-white font-bold">{doneGoodHabits}/{totalGoodHabits}</span>
            </div>
            <div className="w-full bg-zinc-800 h-1.5 rounded-full overflow-hidden">
              <motion.div 
                initial={{ width: 0 }}
        ...
        {totalBadHabits > 0 && (
          <div className="space-y-1.5">
            <div className="flex justify-between text-[10px] uppercase tracking-wider">
              ...
              <span className="text-white font-bold">{resistedBadHabits}/{totalBadHabits}</span>
            </div>
            <div className="w-full bg-zinc-800 h-1.5 rounded-full overflow-hidden">
              <motion.div 
                initial={{ width: 0 }}
