...
export const ShoppingListWidget: React.FC<ShoppingListWidgetProps> = ({ shoppingItems, shoppingCategories }) => {
  ...
  return (
    <div className="bg-white/60 border border-stone-200 rounded-2xl p-6">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-lg font-semibold text-zinc-900">Список покупок</h3>
            <p className="text-sm text-zinc-500">Актуальные пункты</p>
          </div>
        </div>
        <Link to="/shopping-list" className="p-2 hover:bg-stone-100 rounded-lg text-zinc-500 hover:text-zinc-900 transition-colors">
      ...
      <div className="space-y-3">
        {(showMoreShopping ? shoppingItems.filter(i => !i.completed) : shoppingItems.filter(i => !i.completed).slice(0, 3)).map(item => (
          <div key={item.id} className="flex items-center gap-3 p-3 bg-stone-100/40 rounded-xl border border-stone-300/50">
            <div className="w-2 h-2 rounded-full bg-emerald-500" />
            <span className="text-sm text-zinc-700 flex-1 truncate">{item.text}</span>
        ...
        {shoppingItems.filter(i => !i.completed).length > 3 && (
          <button 
            ...
            className="w-full py-2 text-xs text-center text-zinc-500 hover:text-zinc-500 flex items-center justify-center gap-1 transition-colors"
          >
            {showMoreShopping ? (
