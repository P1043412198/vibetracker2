      ...
      {stats.length === 0 ? (
        ...
        <div className="space-y-4">
          {stats.map((stat, index) => (
            <div key={index} className="bg-white/60 p-4 rounded-xl flex items-center justify-between border border-stone-200">
              <div className="flex items-center gap-4">
                <div className="p-2 bg-stone-100 rounded-lg">
                  <Dumbbell className="w-6 h-6 text-emerald-400" />
                </div>
                <div>
                  <h3 className="text-zinc-900 font-medium">{stat.exercise}</h3>
                  <p className="text-zinc-500 text-sm">{format(new Date(stat.date), 'd MMMM yyyy, HH:mm', { locale: ru })}</p>
                </div>
              </div>
