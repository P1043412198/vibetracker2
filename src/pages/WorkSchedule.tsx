...
const SHIFT_COLORS: Record<WorkShiftType, string> = {
  ...
  off: 'bg-stone-100/60 text-zinc-500 border-stone-300/50',
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-zinc-900">График работы</h1>
          <p className="text-zinc-500 text-sm">Настройте свой цикл и планируйте отпуск</p>
        </div>
        <div className="flex gap-2">
          <button
            onClick={() => setIsAddingVacation(true)}
            className="p-2 rounded-xl bg-white border border-stone-200 text-zinc-500 hover:text-zinc-900 transition-colors"
      ...
      <AnimatePresence>
        {showSettings && (
          ...
          >
            <div className="p-6 rounded-3xl bg-white border border-stone-200 space-y-6">
              <div className="flex items-center justify-between">
                <h2 className="text-lg font-bold text-zinc-900">Настройка графика</h2>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      className="w-full bg-stone-50 border border-stone-200 rounded-xl px-4 py-2 text-zinc-900 outline-none focus:border-white/20 transition-colors"
                    />
                  </div>
                  <div className="space-y-2">
                    ...
                    {tempCycle.map((shift, i) => (
                      <div key={i} className="relative group">
                        ...
                        <button
                          ...
                          className="absolute -top-1 -right-1 w-4 h-4 bg-red-500 text-zinc-900 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity"
                        >
                          <X className="w-2 h-2" />
                        </button>
                      </div>
                    ...
                    {getPreviewShifts().map((p, i) => {
                      ...
                      return (
                        <div key={i} className="flex items-center justify-between">
                          <span className="text-xs text-zinc-500">{format(p.date, 'd MMMM (EEEE)', { locale: ru })}</span>
        ...
        {isAddingVacation && (
          ...
          >
            <div className="bg-white border border-stone-200 rounded-3xl p-6 w-full max-w-md shadow-2xl space-y-4">
              <h2 className="text-xl font-bold text-zinc-900">Запланировать отпуск</h2>
              <div className="space-y-3">
                <input
                  type="text"
                  ...
                  className="w-full bg-stone-50 border border-stone-200 rounded-xl px-4 py-2 text-zinc-900 outline-none focus:border-white/20"
                />
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1">
                    ...
                    <input
                      ...
                      className="w-full bg-stone-50 border border-stone-200 rounded-xl px-4 py-2 text-zinc-900 outline-none text-sm"
                    />
                  </div>
                  <div className="space-y-1">
              ...
              <div className="flex gap-3 pt-2">
                <button
                  ...
                  className="flex-1 py-3 bg-stone-100 text-zinc-900 font-bold rounded-2xl hover:bg-stone-200 transition-colors"
                >
                  Отмена
                </button>
                <button
        ...
        <div className="p-4 flex items-center justify-between border-b border-stone-200">
          ...
          <div className="flex gap-1">
            <button
              ...
              className="p-2 rounded-xl hover:bg-stone-100 text-zinc-500 transition-colors"
            >
              <ChevronLeft className="w-5 h-5" />
            </button>
            <button
              onClick={() => setCurrentMonth(new Date())}
              className="px-3 py-1 text-xs font-medium text-zinc-500 hover:text-zinc-900 transition-colors"
            >
              Сегодня
            </button>
            <button
          ...
          {calendarDays.map((day, i) => {
            ...
            return (
              <div
                key={day.toString()}
                className={cn(
                  "min-h-[80px] p-1 border-r border-b border-stone-200 flex flex-col gap-1",
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {workSchedule.vacations.map(vacation => (
              <div key={vacation.id} className="p-4 rounded-2xl bg-white border border-stone-200 flex items-center justify-between group">
                <div className="flex items-center gap-3">
                  <div className="p-2 rounded-xl bg-rose-500/10 text-rose-400">
                    <Plane className="w-5 h-5" />
                  </div>
                  <div>
                    <h4 className="text-sm font-bold text-zinc-900">{vacation.title}</h4>
